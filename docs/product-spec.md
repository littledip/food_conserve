...
| 15 | **Waste-event weighting / impact index** | Once we start logging `disposition: 'wasted'` events for the Food Score, raw counts are not the right unit — wasting a head of lettuce should carry more weight than wasting a tablespoon of dry rice (perishability, environmental footprint, dollar value, household consumption frequency are all candidate inputs). Options: per-category weight table (simple, easy to tune), per-item weight derived from `unitCost × quantity` (depends on Phase 2 grocery API for reliable cost data), or a hybrid environmental-impact index sourced from external data (USDA / FAO). Affects how `wasteRate` rolls up into the Food Score. Revisit once disposition logging has produced enough data to calibrate. |
| 16 | **Unfreezing expiration recalculation** | When an item moves freezer → fridge, `effectiveExpirationDate` should be recomputed as "baseline fridge shelf life minus `fridgeDaysUsedBeforeFreeze`" per the spec rationale in §4.1 (Freeze cycle safety). Today, `moveItem` leaves the freezer date intact on un-freeze, which means a previously-frozen item still displays the freezer expiration in the fridge — incorrect and risky for safety-sensitive items. Requires (a) a baseline fridge shelf life per item or per category, and (b) reliably computing `fridgeDaysUsedBeforeFreeze` from `storageHistory` at the moment of the freeze event. Pairs with deferred decision #2 (storage state machine) since the recalculation only fires on the freezer → fridge transition, and that state machine should make the transition explicit so the recalculation can hook in cleanly. |
